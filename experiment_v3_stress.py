"""
Keystone v3 Experiment - Adversarial Stress Test

Key differences from v2:
1. Real HarmBench adversarial prompts (not synthetic)
2. Multiple training steps per cycle (10-20 steps)
3. DPO-style contrastive learning on refusal vs compliance
4. Measures degradation under real stress

This is a TRUE test of Constitutional Layer stability:
- Real adversarial pressure
- Multiple gradient updates
- Behavioral metrics measured before/after

Usage:
    python experiment_v3_stress.py --cycles 10 --steps 20
    python experiment_v3_stress.py --dry-run
"""
...
class V3CycleRunner:
    """
    Runs v3 adversarial stress cycles.

    Key difference from v2:
    - Real HarmBench adversarial prompts
    - Multiple training steps per cycle (not just 1)
    - DPO-style contrastive loss
    - Measures behavioral degradation under stress
    """
    ...
    def compute_contrastive_loss(
        ...
        This is simplified DPO - we want the model to prefer refusal.
        """
        # Format prompt
        ...
        # Lower loss = more likely, so we minimize refusal_loss and maximize compliance_loss
        beta = 0.1
    ...
    def run_training_steps(
        ...
        """Run multiple adversarial training steps."""
        self.model.train()
        ...
        for step in range(n_steps):
            ...
            # Track Constitutional Layer gradient norms
            cons_norm = 0.0
            cons_count = 0
                ...
                if param.grad is not None and 'lora' in name:
                    for idx in range(6, 11):
                        if f"layers.{idx}." in name:
                            cons_norm += param.grad.norm().item()
                            cons_count += 1
                            break

            if cons_count > 0:
                constitutional_grad_norms.append(cons_norm / cons_count)
            ...
            if verbose and step == 0:
                print(f"    Step {step+1}: loss={loss.item():.4f}, cons_grad={cons_norm:.4f}")
    ...
    def run(
        ...
        """Run a complete v3 adversarial stress cycle."""

        print(f"\n{'='*60}")
        print(f"CYCLE {cycle_num} (v3 - ADVERSARIAL STRESS)")
        print(f"{'='*60}")
        ...
        # 2. Multiple adversarial training steps
        print(f"\n2. Running {n_steps} adversarial training steps with OGPSA...")
...
def run_v3_experiment(
    ...
    """Run the v3 adversarial stress experiment."""
    ...
    for cycle in range(1, num_cycles + 1):
        ...
        # Log incrementally
        if log_dir:
            log_file = Path(log_dir) / "experiment_v3_log.json"
            with open(log_file, "w") as f:
                json.dump(results, f, indent=2)

        # Checkpoint every 5 cycles
        if checkpoint_dir and cycle % 5 == 0:
            ckpt_dir = Path(checkpoint_dir) / f"v3_cycle_{cycle:03d}"
...
def main():
    parser = argparse.ArgumentParser(description="Keystone v3 Adversarial Stress Test")
    parser.add_argument("--cycles", type=int, default=10)
    parser.add_argument("--steps", type=int, default=10)
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--checkpoint-dir", default="./checkpoints_v3")
    parser.add_argument("--log-dir", default="./logs_v3")
    args = parser.parse_args()

    print("=" * 60)
    print("KEYSTONE v3 ADVERSARIAL STRESS TEST")
    print("=" * 60)
