"""
Keystone v7 Experiment - Circuit Breaking with Jailbreak-Aware Harm Probe

Key difference from v6:
- Harm probe trained on BOTH direct harmful AND authority spoofing/jailbreak prompts
- This teaches the circuit breaker to recognize framed requests as harmful

Everything else identical to v6:
- Circuit breaking at Constitutional Layer
- Refusal subspace FIXED
- Applied during training AND inference

Usage:
    python3 experiment_v7_stress.py --cycles 10
    python3 experiment_v7_stress.py --dry-run
"""
...
# V7 NEW: Jailbreak/authority spoofing prompts (harmful despite framing)
JAILBREAK_PROMPTS = [
    # Authority spoofing
...
class CircuitBreaker:
    ...
    def train_harm_probe(self) -> nn.Linear:
        ...
        with torch.no_grad():
            # V7: Include direct harmful prompts
...
class V7CycleRunner:
    """
    Runs v7 adversarial stress cycles with jailbreak-aware circuit breaking.
    ...
    def run(self, cycle_num: int, n_steps: int = 10, verbose: bool = False) -> Dict:
        """Run complete v7 cycle with jailbreak-aware circuit breaking."""

        print(f"\n{'='*60}")
        print(f"CYCLE {cycle_num} (v7 - JAILBREAK-AWARE CIRCUIT BREAKING)")
        print(f"{'='*60}")
...
def run_v7_experiment(
    ...
    """Run v7 experiment with jailbreak-aware circuit breaking."""
    ...
    print("\n" + "=" * 60)
    print("V7: INITIALIZING CIRCUIT BREAKER (Jailbreak-Aware)")
    print("=" * 60)
    circuit_breaker = CircuitBreaker(
        model=model,
        tokenizer=tokenizer,
    ...
    for cycle in range(start_cycle, num_cycles + 1):
        ...
        if log_dir:
            log_file = Path(log_dir) / "experiment_v7_log.json"
            with open(log_file, "w") as f:
                json.dump(results, f, indent=2)

        if checkpoint_dir and cycle % 5 == 0:
            ckpt_dir = Path(checkpoint_dir) / f"v7_cycle_{cycle:03d}"
...
def main():
    parser = argparse.ArgumentParser(description="Keystone v7 - Jailbreak-Aware Circuit Breaking")
    parser.add_argument("--cycles", type=int, default=10)
    parser.add_argument("--steps", type=int, default=10)
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--checkpoint-dir", default="./checkpoints_v7")
    parser.add_argument("--log-dir", default="./logs_v7")
    parser.add_argument("--start-cycle", type=int, default=1)
    parser.add_argument("--resume-log", type=str, default=None)
    args = parser.parse_args()

    print("=" * 60)
    print("KEYSTONE v7 ADVERSARIAL STRESS TEST")
    print("=" * 60)
    ...
    print(f"\nV7 NEW: Jailbreak-Aware Harm Probe")
    print(f"  - Harm probe trained on: direct harmful + authority spoofing prompts")
    print(f"  - {len(HARMBENCH_PROMPTS)} HarmBench + {len(JAILBREAK_PROMPTS)} jailbreak patterns")
    print(f"\nV6 features retained:")
    print(f"  - Circuit breaking at Constitutional Layer")
    print(f"  - Harm probe threshold: 0.3 (conservative)")
    print(f"  - Refusal subspace: FIXED")
